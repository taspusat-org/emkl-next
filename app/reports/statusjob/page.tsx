...
import { statusJobMasukGudang } from '@/constants/statusjob';
...
const ReportMenuPage: React.FC = () => {
  ...
  const { selectedJenisStatusJob } = useSelector(
  ...
  useEffect(() => {
    ...
    const rowData = sessionStorage.getItem('tglstatus');
    if (rowData) {
      try {
        setSaveTglStatus(rowData);
      } catch {
        setSaveTglStatus('');
      }
    }
