...
import FormMenu from './FormJenisseal';
...
import {
  checkValidationJenissealFn,
  getJenissealFn
} from '@/lib/apis/jenisseal.api';
...
const GridJenisseal = () => {
  ...
  const onColumnResize = (index: number, width: number) => {
    ...
    resizeDebounceTimeout.current = setTimeout(() => {
      saveGridConfig(user.id, 'GridJenisseal', [...columnsOrder], newWidthMap);
    }, 300);
  };
  const onColumnsReorder = (sourceKey: string, targetKey: string) => {
    setColumnsOrder((prevOrder) => {
      ...
      saveGridConfig(user.id, 'GridJenisseal', [...newOrder], columnsWidth);
      return newOrder;
    });
  };
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingJenisseal || !hasMore || rows.length === 0) return;
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<IJenisseal>,
  ...
  const onSubmit = async (values: JenissealInput, keepOpenModal = false) => {
    ...
    try {
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('jenisseal');
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleMultipleDelete = async (idsToDelete: number[]) => {
    try {
      ...
      for (const id of idsToDelete) {
        await deleteJenisseal(id as unknown as string);
      }
  ...
  useEffect(() => {
    loadGridConfig(user.id, 'GridJenisseal');
  }, []);
  useEffect(() => {
    setIsFirstLoad(true);
  }, []);
  ...
  }, [allJenisseal, currentPage, filters, isFetchingManually, isDataUpdated]);
